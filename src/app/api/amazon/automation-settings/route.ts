...
export async function PUT(request: NextRequest) {
  ...
  const { data, error } = existing
    ? await supabase.from("amazon_price_automation").update(update).eq("id", existing.id).select().single()
    : await supabase.from("amazon_price_automation").insert(update).select().single();
