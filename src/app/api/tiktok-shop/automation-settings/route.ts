...
export async function PUT(request: NextRequest) {
  ...
  const { enabled, run_time, price_adjustment, active_weekdays } = body;
  ...
  const { data, error } = existing
    ? await supabase.from("tiktok_shop_price_automation").update(update).eq("id", existing.id).select().single()
    : await supabase.from("tiktok_shop_price_automation").insert(update).select().single();
