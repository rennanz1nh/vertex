...
CREATE TABLE IF NOT EXISTS vertex.google_shopping_tokens (
...
ALTER TABLE vertex.google_shopping_tokens ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read google shopping tokens"
  ON vertex.google_shopping_tokens
  FOR SELECT
  TO authenticated
  USING (true);
