...
CREATE TABLE IF NOT EXISTS vertex.search_console_settings (
  ...
  -- (e.g. "https://vertex-rental-cars.vercel.app/" for a URL-prefix property, or "sc-domain:vertex-rental-cars.vercel.app").
  site_url TEXT NOT NULL DEFAULT 'https://vertex-rental-cars.vercel.app/',
  access_token TEXT,
...
ALTER TABLE vertex.search_console_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read search console settings"
  ON vertex.search_console_settings
  FOR SELECT
  TO authenticated
  USING (true);
...
INSERT INTO vertex.search_console_settings (environment)
SELECT 'production'
WHERE NOT EXISTS (SELECT 1 FROM vertex.search_console_settings WHERE environment = 'production');
