...
-- ─── OAuth tokens ──────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.tiktok_shop_tokens (
...
CREATE TABLE IF NOT EXISTS public.tiktok_shop_price_automation (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE public.tiktok_shop_price_automation ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read tiktok price automation"
  ON public.tiktok_shop_price_automation
  FOR SELECT
  TO authenticated
  USING (true);
...
INSERT INTO public.tiktok_shop_price_automation (enabled, run_time, price_adjustment)
SELECT false, '09:00', -0.01
WHERE NOT EXISTS (SELECT 1 FROM public.tiktok_shop_price_automation);

-- ─── Execution history ──────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.tiktok_shop_price_automation_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
ALTER TABLE public.tiktok_shop_price_automation_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read tiktok automation logs"
  ON public.tiktok_shop_price_automation_logs
  FOR SELECT
  TO authenticated
  USING (true);
...
CREATE INDEX IF NOT EXISTS tiktok_shop_price_automation_logs_created_at_idx
  ON public.tiktok_shop_price_automation_logs (created_at DESC);

-- ─── Live progress for an in-flight run ────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.tiktok_shop_price_automation_progress (
...
INSERT INTO public.tiktok_shop_price_automation_progress (id, status, total, completed)
VALUES (1, 'idle', 0, 0)
ON CONFLICT (id) DO NOTHING;

ALTER TABLE public.tiktok_shop_price_automation_progress ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read tiktok automation progress"
  ON public.tiktok_shop_price_automation_progress
  FOR SELECT
  TO authenticated
  USING (true);
...
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS tiktok_line_item_id TEXT;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS tiktok_fulfillment_status TEXT;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS tiktok_status_manual BOOLEAN NOT NULL DEFAULT false;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS comissao_tiktok NUMERIC NOT NULL DEFAULT 0;
