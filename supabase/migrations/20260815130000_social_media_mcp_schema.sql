...
ALTER TABLE vertex.social_media_accounts
...
COMMENT ON COLUMN vertex.social_media_accounts.can_publish IS
...
COMMENT ON COLUMN vertex.social_media_accounts.auto_publish_authorized IS
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS vertex.social_videos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  filename TEXT NOT NULL,
  storage_bucket TEXT NOT NULL DEFAULT 'vertex-social-media',
...
CREATE INDEX IF NOT EXISTS social_videos_status_idx ON vertex.social_videos (status);

ALTER TABLE vertex.social_videos ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Service role manages videos"
  ON vertex.social_videos
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS vertex.social_video_analysis (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  video_id UUID NOT NULL REFERENCES vertex.social_videos(id) ON DELETE CASCADE,
  subject TEXT,
...
CREATE INDEX IF NOT EXISTS social_video_analysis_video_id_idx
  ON vertex.social_video_analysis (video_id, created_at DESC);

ALTER TABLE vertex.social_video_analysis ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Service role manages video analysis"
  ON vertex.social_video_analysis
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS vertex.social_platform_content (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  video_id UUID NOT NULL REFERENCES vertex.social_videos(id) ON DELETE CASCADE,
...
ALTER TABLE vertex.social_platform_content ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Service role manages platform content"
  ON vertex.social_platform_content
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS vertex.social_publications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  video_id UUID NOT NULL REFERENCES vertex.social_videos(id),
  platform TEXT NOT NULL CHECK (platform IN ('instagram', 'tiktok', 'facebook', 'youtube', 'pinterest')),
  account_id UUID NOT NULL REFERENCES vertex.social_media_accounts(id),
  platform_content_id UUID REFERENCES vertex.social_platform_content(id),
...
CREATE INDEX IF NOT EXISTS social_publications_status_idx ON vertex.social_publications (status);
CREATE INDEX IF NOT EXISTS social_publications_scheduled_at_idx ON vertex.social_publications (scheduled_at) WHERE scheduled_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS social_publications_video_id_idx ON vertex.social_publications (video_id);

ALTER TABLE vertex.social_publications ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Service role manages publications"
  ON vertex.social_publications
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS vertex.social_post_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  publication_id UUID NOT NULL REFERENCES vertex.social_publications(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS social_post_metrics_next_sync_idx ON vertex.social_post_metrics (next_sync_at) WHERE next_sync_at IS NOT NULL;

ALTER TABLE vertex.social_post_metrics ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Service role manages post metrics"
  ON vertex.social_post_metrics
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS vertex.social_post_metrics_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  publication_id UUID NOT NULL REFERENCES vertex.social_publications(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS social_post_metrics_history_pub_idx
  ON vertex.social_post_metrics_history (publication_id, recorded_at DESC);

ALTER TABLE vertex.social_post_metrics_history ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Service role manages post metrics history"
  ON vertex.social_post_metrics_history
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS vertex.social_automation_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE vertex.social_automation_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Service role manages automation settings"
  ON vertex.social_automation_settings
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

INSERT INTO vertex.social_automation_settings (id)
SELECT gen_random_uuid()
WHERE NOT EXISTS (SELECT 1 FROM vertex.social_automation_settings);
...
VALUES ('vertex-social-media', 'vertex-social-media', false)
ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS "Vertex - Authenticated users can upload social media videos" ON storage.objects;
CREATE POLICY "Vertex - Authenticated users can upload social media videos"
ON storage.objects
FOR INSERT
WITH CHECK (bucket_id = 'vertex-social-media' AND auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Vertex - Service role manages social media storage" ON storage.objects;
CREATE POLICY "Vertex - Service role manages social media storage"
ON storage.objects
FOR ALL
TO service_role
USING (bucket_id = 'vertex-social-media')
WITH CHECK (bucket_id = 'vertex-social-media');
