...
CREATE TABLE IF NOT EXISTS vertex.ebay_price_automation_progress (
...
INSERT INTO vertex.ebay_price_automation_progress (id, status, total, completed)
VALUES (1, 'idle', 0, 0)
ON CONFLICT (id) DO NOTHING;

ALTER TABLE vertex.ebay_price_automation_progress ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read automation progress"
  ON vertex.ebay_price_automation_progress
  FOR SELECT
  TO authenticated
  USING (true);
