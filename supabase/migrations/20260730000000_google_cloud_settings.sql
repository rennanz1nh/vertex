...
CREATE TABLE IF NOT EXISTS vertex.google_cloud_settings (
...
ALTER TABLE vertex.google_cloud_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read google cloud settings"
  ON vertex.google_cloud_settings
  FOR SELECT
  TO authenticated
  USING (true);
...
INSERT INTO vertex.google_cloud_settings (environment)
SELECT 'production'
WHERE NOT EXISTS (SELECT 1 FROM vertex.google_cloud_settings WHERE environment = 'production');
