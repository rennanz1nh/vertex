...
CREATE TABLE IF NOT EXISTS vertex.ebay_token_refresh_log (
...
ALTER TABLE vertex.ebay_token_refresh_log ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can view token refresh log"
  ON vertex.ebay_token_refresh_log
  FOR SELECT
  TO authenticated
  USING (true);
...
CREATE INDEX IF NOT EXISTS ebay_token_refresh_log_created_at_idx ON vertex.ebay_token_refresh_log (created_at DESC);
