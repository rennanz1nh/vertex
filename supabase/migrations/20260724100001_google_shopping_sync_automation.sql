...
CREATE TABLE IF NOT EXISTS vertex.google_shopping_sync_automation (
...
ALTER TABLE vertex.google_shopping_sync_automation ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read google shopping automation settings"
  ON vertex.google_shopping_sync_automation
  FOR SELECT
  TO authenticated
  USING (true);
...
INSERT INTO vertex.google_shopping_sync_automation (enabled, run_time)
SELECT false, '09:00'
WHERE NOT EXISTS (SELECT 1 FROM vertex.google_shopping_sync_automation);
