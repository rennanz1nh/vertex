...
CREATE TABLE IF NOT EXISTS public.ebay_price_automation_logs (
...
ALTER TABLE public.ebay_price_automation_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read automation logs"
  ON public.ebay_price_automation_logs
  FOR SELECT
  TO authenticated
  USING (true);
...
CREATE INDEX IF NOT EXISTS ebay_price_automation_logs_created_at_idx
  ON public.ebay_price_automation_logs (created_at DESC);
