...
CREATE TABLE IF NOT EXISTS vertex.ebay_eligible_offers_seen (
  listing_id TEXT PRIMARY KEY,
  first_seen_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

ALTER TABLE vertex.ebay_eligible_offers_seen ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can view eligible offers seen"
  ON vertex.ebay_eligible_offers_seen
  FOR SELECT
  TO authenticated
  USING (true);
