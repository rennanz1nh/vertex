...
CREATE TABLE IF NOT EXISTS vertex.amazon_price_automation_logs (
...
ALTER TABLE vertex.amazon_price_automation_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read amazon automation logs"
  ON vertex.amazon_price_automation_logs
  FOR SELECT
  TO authenticated
  USING (true);
...
CREATE INDEX IF NOT EXISTS amazon_price_automation_logs_created_at_idx
  ON vertex.amazon_price_automation_logs (created_at DESC);
