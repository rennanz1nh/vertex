...
CREATE OR REPLACE FUNCTION vertex.notify_admin_new_order()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP TRIGGER IF EXISTS admin_notify_new_order ON vertex.orders;
CREATE TRIGGER admin_notify_new_order
  AFTER INSERT ON vertex.orders
  FOR EACH ROW
  EXECUTE FUNCTION vertex.notify_admin_new_order();
...
CREATE OR REPLACE FUNCTION vertex.notify_admin_delivery_update()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'vertex'
...
DROP TRIGGER IF EXISTS admin_notify_delivery_update ON vertex.orders;
CREATE TRIGGER admin_notify_delivery_update
  AFTER UPDATE ON vertex.orders
  FOR EACH ROW
  EXECUTE FUNCTION vertex.notify_admin_delivery_update();
